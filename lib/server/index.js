...
module.exports = function(backend, serverOptions, clientOptions) {
  ...
  var middleware = BrowserChannelServer(serverOptions, function(client, connectRequest) {
    ...
    function startBrowserChannel() {
      ...
      if (!connectRequest) {
        console.error('no connectRequest');
        console.trace();
      }
      else if (connectRequest.session) client.connectSession = connectRequest.session;
  ...
  wss.on('connection', function (client) {
    ...
    if (!client.upgradeReq) {
      console.error('no client.upgradeReq');
      console.trace();
    }
    else if (client.upgradeReq.session) client.connectSession = client.upgradeReq.session;
